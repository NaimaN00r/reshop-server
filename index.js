...
async function run(){
    try{
        const categoryCollection = client.db('Brand').collection('Category');
        const bookingsCollection = client.db('Brand').collection('bookings');
        const usersCollection = client.db('Brand').collection('users');
        const productsCollection = client.db('Brand').collection('Products');
        const paymentsCollection = client.db('Brand').collection('payments');
        ...
        app.put('/products2/:email',  async (req, res) => {
            const email = req.params.email;
        ...
        app.get('/products1/:isAdvertise', async (req, res) => {
        ...
        app.get('/bookings1/:email/:isWishList', async (req, res) => {
            const email = req.params.email;
...
app.get('/',async(req,res)=>{
    res.send('WatchBox is running');
})



app.listen(port,()=>console.log(`WatchBox is running on ${port}`))
